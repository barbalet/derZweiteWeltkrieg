...
- **Cycles 41-60 complete:** Added order-dice turn completion/end-turn cleanup, spent-die return, retained Ambush/Down preservation, destroyed-unit activation cleanup, morale-quality baseline mapping, pinned-unit order tests with pin and officer modifiers, double-one/double-six handling, structured order-test snapshot fields, and FUBAR Friendly Fire/Panic/Down result reporting with explicit friendly-fire target views.
- **Cycles 61-80 complete:** Added Rally pin removal as a required order action, Rally turn-end gating, Down defensive state, Ambush trigger/cancel semantics, order-specific infantry/artillery/tracked/half-track/wheeled Advance and Run allowances, road movement doubling for vehicles, and terrain-table movement restrictions for rough ground, obstacles, buildings, and roads.
- **Cycles 81-100 complete:** Added vehicle reverse moves, order-dice pivot budgets for tracked/half-track/wheeled vehicles, movement blocker text for UI snapshots, Run-order assault movement, order-test gating before movement/shooting/assault execution, and shooting-procedure state for declared target, range, target reaction, hit/damage resolution, casualties, pins, and morale checks.
- **Cycles 101-120 complete:** Added order-dice shooting hit modifiers for point blank, pins, long range, inexperienced firers, movement, Down targets, small units, and cover; introduced infantry/vehicle damage values with inspectable penetration arithmetic; applied pins from fire, HE/templates, vehicle damage, and FUBAR; and recorded casualty morale rolls, targets, pin/officer modifiers, and failed routing outcomes.
- **Cycles 121-140 complete:** Wrapped normal order-dice shooting behind Fire/Advance orders, normalized converted weapon profiles into range, shots, penetration, HE dice, indirect, team/fixed, shaped-charge, and one-shot fields, enforced indirect-fire minimum range, applied HE hit/pin/penetration profiles, and recorded vehicle armour-arc, long-range, open-topped indirect, and superficial/full/massive penetration outcomes.
- **Cycles 141-160 complete:** Added the order-dice vehicle damage table with Crew Stunned, Immobilized, On Fire, Knocked Out, duplicate-immobilized destruction, and Down transitions; retained knocked-out armoured vehicles as movement-blocking wrecks; replaced order-dice infantry close quarters with loser-destroyed/draw-continuation/regroup outcomes; and added Down/Ambush assault reactions including Ambush opportunity fire before contact.
- **Cycles 161-180 complete:** Added order-dice infantry assaults against vehicles with vehicle Run-move rejection, enclosed-armour nerve tests for infantry without anti-tank assault equipment, vehicle defensive fire, inspectable vehicle hit/damage-class traces, vehicle damage-table application, and regroup after a knockout. The old frontage cap is now fixed-phase legacy behavior only, Swift snapshots include the new assault/order details, and the shared historical board contract can issue explicit Fire/Advance/Run/Ambush/Rally/Down orders while keeping phase advance behind the deprecated `advanceLegacyPhase()` compatibility name.
